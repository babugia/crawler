    ...
    getTeams(link){
        ...
        Crawler.request(link, async function(err, res, body){
            ...
            $('.stats-table.player-ratings-table').find("tbody").find("tr").each(function () {
                ...
                const team_name = $(this).find(".teamCol-teams-overview a").text().trim();

                const teams = {};
                teams.team_id = team_id;
                teams.team_name = team_name;                  

                var target = teams_in.find(temp=>temp.team_id==teams.team_id)
           ...
           jsonfile.writeFile('teams_all.json', teams_in, {spaces: 2}, function (err) {
				console.error(err)
			})
